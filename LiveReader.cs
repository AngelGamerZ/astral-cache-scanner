...
public sealed class LiveReader : IDisposable {
    ...
    public static string ValidateDirectory(string directory) {
        ...
        using(var f=File.OpenRead(exe))using(var sha=SHA256.Create())if(BitConverter.ToString(sha.ComputeHash(f)).Replace("-", "")!=ExpectedHash)throw new InvalidDataException("Diese Wow.exe passt nicht zum geprüften Astral-Client. Bitte den richtigen Ordner wählen; nach einem Clientupdate muss das Profil geprüft werden.");
        return full;
    }
    public LiveReader(string directory) {
        ...
        try {
            ...
            using(var f=File.OpenRead(path)) using(var sha=SHA256.Create()) if(BitConverter.ToString(sha.ComputeHash(f)).Replace("-", "")!=ExpectedHash) throw new InvalidOperationException("Clientdatei verändert: Dieses Profil muss erneut geprüft werden.");
            pid=p.Id;
