...
public static class DesktopTests {
    ...
    public static int Run(string root) {
        ...
        try {
            ...
            var m=new CacheMemory(Path.Combine(dir,"finds.json"));m.Update(s,now);var p=new ProximityCompletion();p.Update(s,m,now);s.objects=new ObjectData[0];Check(p.Update(s,m,now.AddSeconds(.2)).Length==0,"Wait for confirmed absence");Check(p.Update(s,m,now.AddSeconds(.8)).Length==0,"Not enough absence time");Check(p.Update(s,m,now.AddSeconds(1.3)).Length==1 && m.Records.Single().looted,"Absent near player completes without tokens");
            ...
            Check(ReleaseInfo.Describe("{\"tag_name\":\"v1.2.0\"}").StartsWith("Update verfügbar"),"New release");Check(ReleaseInfo.Describe("{\"tag_name\":\"v1.1.0\"}").StartsWith("Aktuell"),"Same release");Check(ReleaseInfo.Describe("{\"tag_name\":\"v1.0.0\"}").StartsWith("Installiert"),"Do not downgrade");
